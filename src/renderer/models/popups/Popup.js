...
export class Popup {
  ...
  dimensions() {
    if (this.data.type === ApiActions.REQUEST_TRANSFER || this.data.type === 'linkApp' || this.data.type === ApiActions.CALLCONTRACTFUNCTION) {
      return {
        width: 800,
        height: 600
      }
  ...
  static transactionSuccess(blockchain, tx, callback) {
    return new Popup(PopupDisplayTypes.POP_IN, new PopupData(PopupTypes.TX_SUCCESS, {
      blockchain,
      tx
    }, callback))
  }
  ...
  static linkOrCreateAccount(keypair, callback) {
    return new Popup(PopupDisplayTypes.POP_IN, new PopupData(PopupTypes.LINK_OR_CREATE_ACCOUNT, {
      keypair
    }, callback))
  }
  ...
  static selector(title, description, icon, items, parser, callback, warning = false) {
    return new Popup(PopupDisplayTypes.POP_IN, new PopupData(PopupTypes.SELECTOR, {
      title,
      description,
      icon,
      items,
  ...
  static snackbar(message, icon, timeout = 3000) {
    return new Popup(PopupDisplayTypes.SNACKBAR, new PopupData('', {
      message,
      icon,
      timeout
    }))
  ...
  static buySellRAM(account, callback) {
    return new Popup(PopupDisplayTypes.POP_IN, new PopupData(PopupTypes.BUY_SELL_RAM, {
      account
    }, callback))
  }

  static delegateResources(account, callback) {
    return new Popup(PopupDisplayTypes.POP_IN, new PopupData(PopupTypes.DELEGATE_RESOURCES, {
      account
    }, callback))
  }
